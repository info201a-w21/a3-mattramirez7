...
jail_pop_rate <- incarceration %>% 
  ...
  filter(year > 1989) %>% 
  select(year, black_jail_pop_rate, aapi_jail_pop_rate, white_jail_pop_rate,
         native_jail_pop_rate, latinx_jail_pop_rate)

jail_chart <- jail_pop_rate %>% 
  pivot_longer(cols = c("black_jail_pop_rate", "aapi_jail_pop_rate", "white_jail_pop_rate",
                        "native_jail_pop_rate", "latinx_jail_pop_rate"),
               names_to = "race",
               values_to = "value")
...
jail_pop_rate_over_time <- jail_chart %>% 
  ...
  labs(x = "Year", y = "Jail Population Rate", title = "Jail Population Rate Over Time in Los Angeles County")
...
los_angeles_blk_jail_prop <- los_angeles_data %>% 
  ...
  labs(x = "Total Jail Population", y = "Black Jail Population", 
